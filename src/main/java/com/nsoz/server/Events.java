...
public class Events {
    ...
    public static int event = SUMMER;
...
//sk 8-3 ngày pnvn
    ...
    //sk he
    public static void huKem(Char p, int number) {
        if (number < 1) {
            p.getService().npcChat(NpcName.TIEN_NU, "Số lượng tối thiểu là 1 cái.");
            return;
        }
        ...
        int kemoc = 3 * number;
        int kemchocolate = 3 * number;
        int kemdau = 3 * number;
        int kemsua = 3 * number;
        int gold = 15 * number;
        int indexKem1 = p.getIndexItemByIdInBag(ItemName.KEM_OC_QUE);
        int indexKem2 = p.getIndexItemByIdInBag(ItemName.KEM_CHOCOLATE);
        int indexKem3 = p.getIndexItemByIdInBag(ItemName.KEM_DAU);
        int indexKem4 = p.getIndexItemByIdInBag(ItemName.KEM_SUA);
        if (indexKem1 == -1 || p.bag[indexKem1] == null || p.bag[indexKem1].getQuantity() < kemoc) {
            p.getService().npcChat(NpcName.TIEN_NU, "Không đủ Kem ốc");
            return;
        }
        if (indexKem2 == -1 || p.bag[indexKem2] == null || p.bag[indexKem2].getQuantity() < kemchocolate) {
            p.getService().npcChat(NpcName.TIEN_NU, "Không đủ Kem Chocolate");
            return;
        }
        if (indexKem3 == -1 || p.bag[indexKem3] == null || p.bag[indexKem3].getQuantity() < kemdau) {
            p.getService().npcChat(NpcName.TIEN_NU, "Không đủ Kem dâu");
            return;
        }
        if (indexKem4 == -1 || p.bag[indexKem4] == null || p.bag[indexKem4].getQuantity() < kemsua) {
            p.getService().npcChat(NpcName.TIEN_NU, "Không đủ Kem sữa");
            return;
        }
        if (p.gold < gold) {
        ...
        p.removeItem(indexKem1, kemoc, true);
        p.removeItem(indexKem2, kemchocolate, true);
        p.removeItem(indexKem3, kemdau, true);
        p.removeItem(indexKem4, kemsua, true);
        p.addGold(-gold);
        Item item = ItemFactory.getInstance().newItem(ItemName.HU_KEM_DAM);
        item.isLock = false;
        item.setQuantity(number);
        p.addItemToBag(item);
    }
 /*
    public static void dieuVai(Char p, int number) {
        if (number < 1) {
            p.getService().npcChat(NpcName.TIEN_NU, "Số lượng tối thiểu là 1 cái.");
            return;
        }
        ...
        int giay = 3 * number;
        int gold = 15 * number;
        int indexTre = p.getIndexItemByIdInBag(ItemName.TRE);
        int indexDay = p.getIndexItemByIdInBag(ItemName.DAY);
        int indexGiay = p.getIndexItemByIdInBag(ItemName.GIAY2);
        int indexMauVe = p.getIndexItemByIdInBag(ItemName.MAU_VE_CAO_CAP);
        
        if (indexTre == -1 || p.bag[indexTre] == null || p.bag[indexTre].getQuantity() < tre) {
            p.getService().npcChat(NpcName.TIEN_NU, "Không đủ Tre");
            return;
        }
        if (indexDay == -1 || p.bag[indexDay] == null || p.bag[indexDay].getQuantity() < day) {
            p.getService().npcChat(NpcName.TIEN_NU, "Không đủ Dây");
            return;
        }
        if (indexGiay == -1 || p.bag[indexGiay] == null || p.bag[indexGiay].getQuantity() < giay) {
            p.getService().npcChat(NpcName.TIEN_NU, "Không đủ Giấy");
            return;
        }
        if (p.gold < gold) {
            p.getService().npcChat(NpcName.TIEN_NU, "Không đủ xu");
        ...
        p.removeItem(indexTre, tre, true);
        p.removeItem(indexDay, day, true);
        p.removeItem(indexGiay, giay, true);
        Item item = ItemFactory.getInstance().newItem(ItemName.DIEU_VAI);
        item.isLock = false;
        p.addGold(-gold);
    ...
    public static void dieuGiay(Char p, int number) {
        if (number < 1) {
            p.getService().npcChat(NpcName.TIEN_NU, "Số lượng tối thiểu là 1 cái.");
            return;
        }
        ...
        p.removeItem(indexDay, day, true);
        p.removeItem(indexGiay, giay, true);
        Item item = ItemFactory.getInstance().newItem(ItemName.DIEU_GIAY);
        item.isLock = false;
        item.setQuantity(number);
        p.addItemToBag(item);
       p.addCoin(-coin);
