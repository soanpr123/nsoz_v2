...
public class Service extends AbsService {
 ...
 /*   public void addVnd(int add) {
        try {
            Message ms = messageSubCommand(CMD.ME_UP_VND);
            DataOutputStream ds = ms.writer();
            ds.writeInt(add);
            ds.flush();
            sendMessage(ms);
    ...
    public void loadVnd() {
        try {
            Message ms = messageSubCommand(CMD.ME_LOAD_VND);
            DataOutputStream ds = ms.writer();
            ds.writeInt(player.user.vnd);
            ds.flush();
            sendMessage(ms);
            ms.cleanup();
        } catch (Exception ex) {
    ...
    public  void addEffectChar(final Session conn, final byte b, final int vId, final short id, final int timelive, final int miliSecondWait) {
        if (conn == null) return;

        Message msg = null;
        boolean isHead = (id >= 12 && id <= 20) || id == 37 || id == 58 || (id >= 50 && id <= 116) || id == 45 || id == 40 || id == 74 || id == 77;
        try {
            msg = new Message(125);
    ...
    public void sendImgEffect(Message msg) {
        try {
            ...
            Logger.getLogger("res/Effect/x" + session.zoomLevel + "/ImgEffect/ImgEffect " + id + ".png");
            byte[] ab = GameData.getInstance().loadFile("res/Effect/x" + session.zoomLevel + "/ImgEffect/ImgEffect " + id + ".png");
    ...
    public void sendEffectData(Message msg) {
        try {
            short id = msg.reader().readShort();
            final byte[] eff = GameData.getInstance().loadFile("res/Effect/x" + session.zoomLevel + "/DataEffect/" + id);
            if (eff != null) {
